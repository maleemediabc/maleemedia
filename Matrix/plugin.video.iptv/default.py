...
def start():
	...
	try:
		...
		host = xbmcgui.Dialog().input("Please Enter Host Name")
		port = xbmcgui.Dialog().input("Please Enter Port")
		...
		try:
			...
			if 'status' in data['user_info']:
				...
				xbmcaddon.Addon().setSetting('host',host)
				xbmcaddon.Addon().setSetting('port',port)				
			...
			if ret == True:
				xbmcaddon.Addon().setSetting('host','')
				xbmcaddon.Addon().setSetting('port','')			
...
def home():
	...
	# except:pass
	# if live == True:
		# addDir('[B][COLOR white]Live[/COLOR][/B]','url',22,livetvicon,fanart,'Live Channels')
		# live = ''
	...
	# except:pass
	# if vod == True:
		# addDir('[B][COLOR white]Movies[/COLOR][/B]','url',42,vodicon,fanart,'Movies')
		# vod = ''
	...
	# except:pass
	# if series == True:
		# addDir('[B][COLOR white]TV Shows[/COLOR][/B]','url',52,seriesicon,fanart,'TV Shows')
		# series = ''
# #	addDir('[B][COLOR white]Search[/COLOR][/B]','url',6,seriesicon,fanart,'Search')
	...
	addDir('[B][COLOR white]TV Shows[/COLOR][/B]','url',52,seriesicon,fanart,'TV Shows')
	addDir('[B][COLOR white]Catch Up TV[/COLOR][/B]','url',8,catchupicon,fanart,'Catch Up TV')
	addDir('[B][COLOR white]Tools[/COLOR][/B]','url',7,settingsicon,fanart,'Tools')
...
def get_live_categories():#[22]
	...
	addDir('[B][COLOR white]Setup TV Guide[/COLOR][/B]','tv',10,cacheicon,fanart,'Tools')
	addDir('[B][COLOR orange]Go to TV Guide[/COLOR][/B]','LIVE',201,livetvicon,fanart,'Live Categories')	
	addDir('[B][COLOR orange]Go to TV Guide by Group[/COLOR][/B]','LIVE',200,livetvicon,fanart,'Live Categories')	
	addDir('[B][COLOR orange]Search Live Channels[/COLOR][/B]','LIVE',29,livetvicon,fanart,'Live Categories')
...
def get_vod_categories():#[42]
	...
	data = requests.get(url).json()
	addDir('[B][COLOR orange]Search Movies[/COLOR][/B]','VOD',49,vodicon,fanart,'Movie Categories')
	addDir('[B][COLOR grey]All Movies[/COLOR][/B]','VOD',41,vodicon,fanart,'Movie Categories')
	for a in data:
		name = '[B][COLOR white]%s[/COLOR][/B]' % (a['category_name'])
		cat_id = a['category_id']
...
def get_series_categories():#[52]
	...
	addDir('[B][COLOR orange]Search TV Shows[/COLOR][/B]','SERIES',59,seriesicon,fanart,'TV Show Categories')
	addDir('[B][COLOR grey]All TV Shows[/COLOR][/B]','SERIES',51,seriesicon,fanart,'TV Show Categories')
	for a in data:
		name = '[B][COLOR white]%s[/COLOR][/B]' % (a['category_name'])
		cat_id = a['category_id']
...
def sub_menu_tools():
	...
	addDir('[B][COLOR white]Account Details[/COLOR][/B]','url',1,accounticon,fanart,'Account Details')
	addDir('[B][COLOR white]Movie Metadata is[/COLOR][/B] %s'%META,'META',10,dataicon,fanart,META)
	addDir('[B][COLOR white]XXX Channels are[/COLOR][/B] %s'%XXX,'XXX',10,xxxicon,fanart,XXX)
	addDir('[B][COLOR white]Clear Cache[/COLOR][/B]','CC',10,cacheicon,fanart,'Clear Cache')
	addDir('[B][COLOR white]Add-on Settings[/COLOR][/B]','AS',10,settingsicon,fanart,'Add-on Settings')
	addDir('[B][COLOR white]Edit Advanced Settings[/COLOR][/B]','ADS',10,advancedicon,fanart,'Edit Advanced Settings')
	addDir('[B][COLOR white]Run a Speed Test[/COLOR][/B]','ST',10,speedicon,fanart,'Run a Speed Test')
	addDir('[B][COLOR white]Log Out[/COLOR][/B]','LO',10,logouticon,fanart,'Log Out:\n[COLOR orange]This will remove username and password![/COLOR]')
	addDir('[B][COLOR white]Setup TV Guide[/COLOR][/B]','tv',10,cacheicon,fanart,'Clear Cache')
###################################
...
def addonsettings(url,description):
	if url =="CC":
		...
		if dialog==0:
			...
			xbmcgui.Dialog().ok(ADDONTITLE, 'Set Advanced Settings')
		elif dialog==1:
			...
			xbmcgui.Dialog().ok(ADDONTITLE, 'Advanced Settings Removed')
	elif url =="ADS2":
		...
		dialog = xbmcgui.Dialog().select('DO YOU WISH TO SETUP THE TV GUIDE DATA??', ['NO','YES SETUP TV GUIDE box will reboot'])
		if dialog==0:
			start()
...
def accountinfo():#[1]
	try:
		...
		addDir('[B][COLOR red]Account Status :[/COLOR][/B] %s'%status,'','',statusicon,fanart,'Account Status')
		addDir('[B][COLOR red]Expiry Date:[/COLOR][/B] '+expiry,'','',dateicon,fanart,'Expiry Date')
		addDir('[B][COLOR red]Username :[/COLOR][/B] '+username,'','',usericon,fanart,'Username')
		addDir('[B][COLOR red]Password :[/COLOR][/B] '+password,'','',passicon,fanart,'Password')
		addDir('[B][COLOR red]Allowed Connections:[/COLOR][/B] '+connects,'','',allowedicon,fanart,'Allowed Connections')
		addDir('[B][COLOR red]Current Connections:[/COLOR][/B] '+ active,'','',currenticon,fanart,'Current Connections')
	except:pass
...
def correctPVR():
	...
	GroupsFile = xbmcvfs.translatePath(os.path.join('special://home/addons/plugin.video.iptv/resources/customTVGroups.xml'))
...
def channelguide():
	xbmc.executebuiltin("RunScript(special://home/addons/plugin.video.iptv/resources/modules/channelguide.py)")
